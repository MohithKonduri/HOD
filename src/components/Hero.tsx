import { Mail, Github, Linkedin, User as UserIcon, Building2, IdCard, Phone, GraduationCap, Globe, Fingerprint, BookOpen, FileText, Newspaper } from 'lucide-react';
...
const StatsCard: React.FC<{
    ...
    const items = [
        { label: 'Journals', value: stats.journals, icon: FileText, color: 'text-emerald-600', bg: 'bg-emerald-50' },
        { label: 'Conferences', value: stats.conferences, icon: Newspaper, color: 'text-blue-600', bg: 'bg-blue-50' },
        { label: 'Books', value: stats.books, icon: BookOpen, color: 'text-purple-600', bg: 'bg-purple-50' }
    ];

    return (
        <div className="relative group max-w-md">
            {/* Stack Effect Backgrounds */}
            <div className="absolute inset-x-2 -bottom-2 h-full bg-white rounded-2xl shadow-sm border border-slate-200 opacity-60 z-0"></div>
            <div className="absolute inset-x-4 -bottom-4 h-full bg-white rounded-2xl shadow-sm border border-slate-200 opacity-30 -z-10"></div>

            {/* Main Card */}
            <div className="relative z-10 bg-white rounded-2xl shadow-xl border border-slate-100 p-6 grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-4">
                {items.map((item, idx) => {
                    const Icon = item.icon;
                    return (
                        <div key={idx} className="flex flex-col items-center text-center">
                            <div className={`p-3 rounded-xl ${item.bg} mb-3 group-hover:scale-110 transition-transform duration-300`}>
                                <Icon className={`w-6 h-6 ${item.color}`} strokeWidth={2} />
                            </div>
                            <span className="text-3xl font-black text-slate-800 mb-1">{item.value}</span>
        ...
        <section id="basicInfo" className="min-h-[85vh] relative flex flex-col md:flex-row items-center overflow-hidden mesh-gradient">
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto px-6 py-12 md:py-20 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                ...
                >
                    ...
                    {/* Stats Card - Back Below Name */}
                    {publicationStats && (publicationStats.journals > 0 || publicationStats.conferences > 0 || publicationStats.books > 0) && (
                        <div className="relative mt-6 -ml-2 mb-8 transform hover:scale-105 transition-transform duration-300 origin-left">
                            <StatsCard
                                stats={{
